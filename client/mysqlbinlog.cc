    ...
    */
    if (gtid_state_validator && !was_first_glle_processed && glev->count)
    {
      ...
      if (domain_window_gtid_filter &&
          !domain_window_gtid_filter->get_num_start_gtids())
      {
        /*
        ...
        */
        ...
        size_t n_stop_gtids= domain_window_gtid_filter->get_num_stop_gtids();
    ...
    else if (n_start_gtid_ranges > 0)
    {
      ...
      for (gtid_idx = 0; gtid_idx < n_start_gtid_ranges; gtid_idx++)
      {
        ...
        if (start_gtid->seq_no &&
            domain_window_gtid_filter->add_start_gtid(start_gtid))
        {
          my_free(start_gtids);
          return 1;
        }
