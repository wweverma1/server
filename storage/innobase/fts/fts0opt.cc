...
#ifdef WITH_WSREP
/** whether Galera snapshot transfer has frozen writes */
Atomic_relaxed<bool> wsrep_sst_fts_sync_paused;
#else
constexpr bool wsrep_sst_fts_sync_paused= false;
#endif
...
static void fts_optimize_callback(void *)
{
	...
	while (!done && srv_shutdown_state <= SRV_SHUTDOWN_INITIATED) {
		...
		if (!done
			...
			to optimize. */
			if (UNIV_UNLIKELY(wsrep_sst_fts_sync_paused)) {
retry_later:
